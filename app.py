...
def generate_certificate(first_name: str, last_name: str, earned: int, max_pts: int, pct: int) -> bytes:
    ...
    pdf.line(lx, pdf.get_y() + 1, lx + name_w, pdf.get_y() + 1)
    pdf.ln(9)
    ...
    pdf.set_font("Helvetica", "B", 13)
    pdf.set_text_color(*BLACK)
    pdf.cell(0, 9, "General Compliance Onboarding Training", align="C", ln=True)
    ...
    pdf.set_text_color(*BLACK)
    pdf.set_y(pill_y + 4)
    pdf.cell(0, 9, f"Score: {earned} / {max_pts} points", align="C", ln=True)
    pdf.set_font("Helvetica", "", 10)
    pdf.cell(0, 7, f"{pct}%  -  PASSED", align="C", ln=True)

    # ── Date ──────────────────────────────────────────────────────────────────
    pdf.ln(10)
    pdf.set_font("Helvetica", "", 9)
    pdf.set_text_color(*DGRAY)
    pdf.cell(0, 7, f"Date of completion: {date.today().strftime('%B %d, %Y')}", align="C", ln=True)

    # ── Lime bottom accent ─────────────────────────────────────────────────────
    pdf.set_fill_color(*LIME)
    pdf.rect(15, H - 29, W - 30, 8, "F")

    # ── Black footer bar ───────────────────────────────────────────────────────
    pdf.set_fill_color(*BLACK)
    pdf.rect(15, H - 21, W - 30, 6, "F")
    pdf.set_font("Helvetica", "", 7)
    pdf.set_text_color(*WHITE)
    pdf.set_y(H - 20)
    pdf.cell(0, 6, "Finax, o.c.p., a.s.   |   compliance@finax.eu   |   www.finax.eu", align="C")
