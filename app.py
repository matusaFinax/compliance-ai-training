...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if test_type == "onboarding":
        ...
        if passed:
            ...
            st.markdown(
                f'<div class="result-fail">❌ TEST FAILED — {pct}%<br>({earned} / {max_pts} points) — Minimum required: 60%</div>',
                unsafe_allow_html=True,
            )
        ...
        if pct >= 80:
            ...
            badge, msg = "🥉 Compliance Rookie", "Needs improvement. Please review the training materials and try again."
        ...
        c3.metric("Result", "✅ Passed" if passed else "❌ Failed")
