...
- Be FAIR and CHARITABLE. If the answer captures the core idea — even briefly or imperfectly worded — give full credit.
- Score 2 if the main correct action(s) are present, even without listing every detail.
- Score 1 if some correct elements are there but something important is missing or unclear.
- Score 0 only if the answer is clearly wrong or shows a fundamental misunderstanding.
- Set needs_clarification=true when score=1 AND the answer is ambiguous or might be correct but incomplete phrasing. Ask one short, specific question to let the employee clarify. Do NOT ask for clarification if score=0 (clearly wrong) or score=2 (clearly correct).

Respond ONLY with a valid JSON object — no other text:
{
  "score": 0 or 1 or 2,
  "needs_clarification": true or false,
  "clarification_question": "One short, friendly follow-up question asking about the specific missing/ambiguous part. Only set this if needs_clarification=true, otherwise empty string.",
  "feedback": "2-3 sentences of personalised feedback — acknowledge what was right, explain what was missing or incorrect",
...
CLARIFY_SYSTEM_PROMPT = """You are a compliance answer evaluator for Finax.
The employee answered a compliance scenario. Their initial answer was partially correct (score=1).
You asked a clarifying follow-up question. Now you have their clarification.
Re-evaluate the COMBINED answer (original + clarification) and give a FINAL score.

RULES:
- If the clarification fills in the missing part, upgrade to score=2.
- If the clarification still misses the key point, keep score=1.
- If the clarification reveals a misunderstanding, keep score=1 or lower to 0.
- Be encouraging — the employee tried to clarify.

Respond ONLY with a valid JSON object — no other text:
{
  "score": 0 or 1 or 2,
  "feedback": "2-3 sentences acknowledging their clarification and giving the final verdict. Be warm and specific.",
  "what_right": "What they got right across both answers (1 sentence)",
  "what_missing": "What was still missing, if anything (1 sentence, or 'Nothing' if score=2)"
}"""


# ── Session state ─────────────────────────────────────────────────────────────
def _img_buf(path: str):
    """Load image via Pillow, composite onto white bg → JPEG BytesIO."""
    from PIL import Image
    from io import BytesIO
    img = Image.open(path).convert("RGBA")
    bg = Image.new("RGB", img.size, (255, 255, 255))
    bg.paste(img, mask=img.split()[3])   # transparent → white
    buf = BytesIO()
...
def generate_certificate(first_name: str, last_name: str, earned: int, max_pts: int, pct: int) -> bytes:
    ...
    pdf = FPDF()
    pdf.set_auto_page_break(False)          # prevent blank second page
    pdf.add_page()
    ...
    pdf.rect(15, 15, 180, 18, "F")

    # ── "Compliance Training Certificate" label (right of header) ─────────────
    pdf.set_font("Helvetica", "", 7)
    pdf.set_text_color(*BLACK)
    pdf.set_xy(15, 20)
    pdf.cell(180, 8, "Compliance Training Certificate", align="R")

    # ── Finax logo (on white, below header) ────────────────────────────────────
    dots_path = os.path.join(BASE_DIR, "finax_dots.png")
    if os.path.exists(dots_path):
        pdf.image(_img_buf(dots_path), x=20, y=36, w=55)

    # ── Mascot (centered, prominent) ──────────────────────────────────────────
    mascot_path = os.path.join(BASE_DIR, "mascot.png")
    if os.path.exists(mascot_path):
        pdf.image(_img_buf(mascot_path), x=W // 2 - 27, y=34, w=54)

    # ── "This is to certify that" ─────────────────────────────────────────────
    pdf.set_font("Helvetica", "", 11)
    pdf.set_text_color(*DGRAY)
    pdf.set_xy(15, 100)
    pdf.cell(180, 8, "This is to certify that", align="C")

    # ── Name ──────────────────────────────────────────────────────────────────
    pdf.set_font("Helvetica", "B", 28)
    pdf.set_text_color(*BLACK)
    pdf.set_xy(15, 112)
    pdf.cell(180, 14, f"{first_name} {last_name}", align="C")

    # ── Lime underline ─────────────────────────────────────────────────────────
    lx = (W - 120) / 2
    pdf.set_draw_color(*LIME)
    pdf.set_line_width(2.5)
    pdf.line(lx, 128, lx + 120, 128)

    # ── "has successfully completed the" ──────────────────────────────────────
    pdf.set_font("Helvetica", "", 11)
    pdf.set_text_color(*DGRAY)
    pdf.set_xy(15, 133)
    pdf.cell(180, 8, "has successfully completed the", align="C")

    # ── Training title ─────────────────────────────────────────────────────────
    pdf.set_font("Helvetica", "B", 13)
    pdf.set_text_color(*BLACK)
    pdf.set_xy(15, 145)
    pdf.cell(180, 9, "General Compliance Onboarding Training", align="C")

    # ── Score pill ─────────────────────────────────────────────────────────────
    pill_w, pill_h, pill_y = 110, 28, 168
    pill_x = (W - pill_w) / 2
    pdf.set_fill_color(*LIME)
    pdf.rect(pill_x, pill_y, pill_w, pill_h, "F")
    pdf.set_font("Helvetica", "B", 14)
    pdf.set_text_color(*BLACK)
    pdf.set_xy(15, pill_y + 4)
    pdf.cell(180, 9, f"Score: {earned} / {max_pts} points", align="C")
    pdf.set_font("Helvetica", "", 10)
    pdf.set_xy(15, pill_y + 15)
    pdf.cell(180, 7, f"{pct}%  -  PASSED", align="C")

    # ── Date ──────────────────────────────────────────────────────────────────
    pdf.set_font("Helvetica", "", 9)
    pdf.set_text_color(*DGRAY)
    pdf.set_xy(15, 208)
    pdf.cell(180, 7, f"Date of completion: {date.today().strftime('%B %d, %Y')}", align="C")

    # ── Lime accent bar ────────────────────────────────────────────────────────
    pdf.set_fill_color(*LIME)
    pdf.rect(15, 270, 180, 7, "F")

    # ── Black footer ───────────────────────────────────────────────────────────
    pdf.set_fill_color(*BLACK)
    pdf.rect(15, 277, 180, 5, "F")
    pdf.set_font("Helvetica", "", 6)
    pdf.set_text_color(*WHITE)
    pdf.set_xy(15, 277)
    pdf.cell(180, 5, "Finax, o.c.p., a.s.   |   compliance@finax.eu   |   www.finax.eu", align="C")
...
def evaluate_with_clarification(question: dict, original_answer: str, clarification: str) -> dict:
    client = anthropic.Anthropic(api_key=st.session_state.api_key)
    user_msg = (
        f"SCENARIO:\n{question['scenario']}\n\n"
        f"CORRECT APPROACH:\n{question['correct_approach']}\n\n"
        f"EMPLOYEE'S ORIGINAL ANSWER:\n{original_answer}\n\n"
        f"YOUR FOLLOW-UP QUESTION:\n{st.session_state.clarification_question}\n\n"
        f"EMPLOYEE'S CLARIFICATION:\n{clarification}"
    )
...
def reset_game():
    keys = ["game_state", "test_type", "first_name", "last_name", "num_questions", "questions",
            ...
            "user_answer", "clarification_needed", "clarification_question",
            "clarification_answered", "clarification_answer", "history"]
    for k in keys:
...
# ══════════════════════════════════════════════════════════════════════════════
            ...
            if not user_ans.strip():
                ...
                with st.spinner("🤖 AI is evaluating your answer..."):
                    try:
                        ...
                        needs_clarify = ev.get("needs_clarification", False) and ev["score"] == 1
                        st.session_state.clarification_needed = needs_clarify
                        st.session_state.clarification_question = ev.get("clarification_question", "") if needs_clarify else ""
                        st.session_state.clarification_answered = False
                        # Only add score now if no clarification pending
    ...
    # ── STATE 2: Clarification needed ─────────────────────────────────────────
    elif st.session_state.answered and st.session_state.clarification_needed and not st.session_state.clarification_answered:
        st.markdown("**Your answer:**")
        st.info(st.session_state.user_answer)

        st.markdown(
            f'<div class="feedback-partial">🤔 <strong>One follow-up question:</strong><br><br>'
            f'{st.session_state.clarification_question}</div>',
            unsafe_allow_html=True,
        )
        st.markdown("")
        clarify_ans = st.text_area(
            label="Clarification",
            placeholder="Clarify your answer here...",
            height=100,
            label_visibility="collapsed",
        )
        if st.button("✅ Submit Clarification", use_container_width=True, type="primary"):
            if not clarify_ans.strip():
                st.warning("Please write your clarification before submitting.")
            else:
                with st.spinner("🤖 AI is reviewing your clarification..."):
                    try:
                        ev2 = evaluate_with_clarification(q, st.session_state.user_answer, clarify_ans)
                        st.session_state.evaluation = ev2
                        st.session_state.clarification_answer = clarify_ans
                        st.session_state.clarification_answered = True
                        # Score: full marks if clarification was correct, else keep at 1
                        final_score = ev2["score"]  # can be 0, 1 or 2
                        ...
                        st.session_state.history.append({
                            ...
                            "clarification": clarify_ans,
                            "score": final_score,
                            "feedback": ev2["feedback"],
                            "what_right": ev2.get("what_right", ""),
                            "what_missing": ev2.get("what_missing", ""),
                            "consequence": q["consequence"],
                            "correct_approach": q["correct_approach"],
                            "takeaway": q["takeaway"],
                        })
    ...
    # ── STATE 3: Final feedback ────────────────────────────────────────────────
        ...
        if st.session_state.clarification_answered and st.session_state.clarification_answer:
            st.markdown("**Your clarification:**")
            st.info(st.session_state.clarification_answer)
        ...
        st.markdown(f'<p class="{score_css[score]}">{score_label[score]}</p>', unsafe_allow_html=True)
        ...
        # Consequence — only if wrong or partial
        if score < 2:
            st.markdown(
